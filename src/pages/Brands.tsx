      ...
      <div className="max-w-[1000px] mx-auto px-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {brands.map((brand, i) => {
            ...
            return (
              <motion.div key={brand} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: i * 0.1 }}>
                <Link to={`/brands/${brand.toLowerCase()}`} className="group border border-border rounded-2xl p-10 flex flex-col items-center justify-center hover:border-foreground/30 hover:shadow-lg transition-all duration-300 no-underline aspect-square">
